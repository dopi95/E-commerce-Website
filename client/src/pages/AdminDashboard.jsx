  ...
  return (
    <div className='w-full max-w-full overflow-x-hidden'>
      <div className='mb-6 sm:mb-8'>
        ...
        <p className='text-gray-600 text-sm sm:text-base'>Welcome back, {user?.name}! Here's what's happening with your store.</p>
      </div>
      ...
      <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 sm:gap-6 mb-6 sm:mb-8'>
        <div className='bg-blue-50 border border-blue-200 rounded-lg p-4 sm:p-6'>
          <div className='flex items-center justify-between'>
            <div className='min-w-0 flex-1'>
              ...
              <p className='text-xl sm:text-2xl font-bold text-blue-800 truncate'>{stats.totalProducts}</p>
            </div>
        ...
        <div className='bg-green-50 border border-green-200 rounded-lg p-4 sm:p-6'>
          <div className='flex items-center justify-between'>
            <div className='min-w-0 flex-1'>
              <p className='text-green-600 text-xs sm:text-sm font-medium'>Categories</p>
              <p className='text-xl sm:text-2xl font-bold text-green-800 truncate'>{stats.totalCategories}</p>
            </div>
        ...
        <div className='bg-purple-50 border border-purple-200 rounded-lg p-4 sm:p-6'>
          <div className='flex items-center justify-between'>
            <div className='min-w-0 flex-1'>
              <p className='text-purple-600 text-xs sm:text-sm font-medium'>Total Users</p>
              <p className='text-xl sm:text-2xl font-bold text-purple-800 truncate'>{stats.totalUsers}</p>
            </div>
        ...
        <div className='bg-orange-50 border border-orange-200 rounded-lg p-4 sm:p-6'>
          <div className='flex items-center justify-between'>
            <div className='min-w-0 flex-1'>
              <p className='text-orange-600 text-xs sm:text-sm font-medium'>Total Orders</p>
              <p className='text-xl sm:text-2xl font-bold text-orange-800 truncate'>{stats.totalOrders}</p>
            </div>
            <div className='bg-orange-100 p-2 sm:p-3 rounded-full flex-shrink-0 ml-2'>
              <svg className='w-4 h-4 sm:w-6 sm:h-6 text-orange-600' fill='currentColor' viewBox='0 0 20 20'>
        ...
        <div className='bg-red-50 border border-red-200 rounded-lg p-4 sm:p-6'>
          <div className='flex items-center justify-between'>
            <div className='min-w-0 flex-1'>
              <p className='text-red-600 text-xs sm:text-sm font-medium'>Revenue</p>
              <p className='text-lg sm:text-2xl font-bold text-red-800 truncate'>{DisplayPriceInRupees(stats.totalRevenue)}</p>
            </div>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6'>
        <div className='bg-white border rounded-lg p-4 sm:p-6 hover:shadow-lg transition-shadow'>
          ...
          <div className='space-y-3'>
            <Link to='/dashboard/upload-product' className='block w-full text-left px-3 sm:px-4 py-2 bg-blue-50 hover:bg-blue-100 rounded-lg text-blue-700 font-medium transition-colors text-sm sm:text-base'>
              Add New Product
            </Link>
            <Link to='/dashboard/category' className='block w-full text-left px-3 sm:px-4 py-2 bg-green-50 hover:bg-green-100 rounded-lg text-green-700 font-medium transition-colors text-sm sm:text-base'>
              Manage Categories
            </Link>
            <Link to='/dashboard/orders' className='block w-full text-left px-3 sm:px-4 py-2 bg-purple-50 hover:bg-purple-100 rounded-lg text-purple-700 font-medium transition-colors text-sm sm:text-base'>
              View All Orders
            </Link>
          </div>
        </div>
        ...
        <div className='bg-white border rounded-lg p-4 sm:p-6'>
          <h3 className='text-base sm:text-lg font-semibold text-gray-800 mb-4'>Store Statistics</h3>
        ...
        <div className='bg-white border rounded-lg p-4 sm:p-6'>
          <h3 className='text-base sm:text-lg font-semibold text-gray-800 mb-4'>Latest Products</h3>
          <div className='space-y-3 text-xs sm:text-sm'>
            {topProducts.length > 0 ? (
              topProducts.map((product, index) => (
                <div key={index} className='flex justify-between items-center gap-2'>
                  <span className='text-gray-600 truncate flex-1'>{product.name}</span>
                  <span className='font-medium text-green-600 flex-shrink-0'>{DisplayPriceInRupees(product.price)}</span>
                </div>
              ))
            ) : (
